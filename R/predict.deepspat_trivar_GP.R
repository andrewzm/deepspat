...
#' @export
#' @examples
#' df <- data.frame(s1 = rnorm(100), s2 = rnorm(100),
...
#' \dontrun{
#' \dontrun{d <- deepspat_trivar_GP(f = z1 + z2 + z3 ~ s1 + s2 - 1,
#'                                  data = df, g = ~ 1,
#'                                  layers = layers, method = "REML",
#'                                  family = "matern_nonstat_symm",
#'                                  nsteps = 100L)}
#'   pred <- predict.deepspat_trivar_GP(d, dfnew)
#' }
predict.deepspat_trivar_GP <- function(object, newdata, ...) {
  ...
  if (d$family == "matern_nonstat_asymm"){
    ...
    s_in3 <- scale_0_5_tf(s_tf, d$scalings_asym[[1]]$min, d$scalings_asym[[1]]$max)
      ...
      for(i in 1:d$nlayers_asym) {
        if (d$layers_asym_2[[i]]$name == "AFF_2D"){
          sout1_tf <- tf$reshape(d$layers_asym_2[[i]]$pars[[1]] + d$layers_asym_2[[i]]$pars[[2]] * h_tf2_asym[[i]][,1] + d$layers_asym_2[[i]]$pars[[3]] * h_tf2_asym[[i]][,2], c(nrow(h_tf2_asym[[i]][,1]),1L))
          sout2_tf <- tf$reshape(d$layers_asym_2[[i]]$pars[[4]] + d$layers_asym_2[[i]]$pars[[5]] * h_tf2_asym[[i]][,1] + d$layers_asym_2[[i]]$pars[[6]] * h_tf2_asym[[i]][,2], c(nrow(h_tf2_asym[[i]][,1]),1L))
          h_tf2_asym[[i + 1]] <- tf$concat(list(sout1_tf, sout2_tf), axis=1L) %>%
            scale_0_5_tf(smin_tf = d$scalings_asym[[i + 1]]$min,
                         smax_tf = d$scalings_asym[[i + 1]]$max)
          
          sout1_tf <- tf$reshape(d$layers_asym_3[[i]]$pars[[1]] + d$layers_asym_3[[i]]$pars[[2]] * h_tf3_asym[[i]][,1] + d$layers_asym_3[[i]]$pars[[3]] * h_tf3_asym[[i]][,2], c(nrow(h_tf3_asym[[i]][,1]),1L))
          sout2_tf <- tf$reshape(d$layers_asym_3[[i]]$pars[[4]] + d$layers_asym_3[[i]]$pars[[5]] * h_tf3_asym[[i]][,1] + d$layers_asym_3[[i]]$pars[[6]] * h_tf3_asym[[i]][,2], c(nrow(h_tf3_asym[[i]][,1]),1L))
          h_tf3_asym[[i + 1]] <- tf$concat(list(sout1_tf, sout2_tf), axis=1L) %>%
            scale_0_5_tf(smin_tf = d$scalings_asym[[i + 1]]$min,
                         smax_tf = d$scalings_asym[[i + 1]]$max)
        }
        else{
    ...
    K_obs <- tf$concat(list(tf$concat(list(K_obs_11, K_obs_12, K_obs_13), axis=1L),
                            tf$concat(list(tf$matrix_transpose(K_obs_12), K_obs_22, K_obs_23), axis=1L),
                            tf$concat(list(tf$matrix_transpose(K_obs_13), tf$matrix_transpose(K_obs_23), K_obs_33), axis=1L)
                            ), axis=0L)
    ...
    K_star <- tf$concat(list(tf$concat(list(K_star_11, K_star_12, K_star_13), axis=1L),
                             tf$concat(list(tf$matrix_transpose(K_star_12), K_star_22, K_star_23), axis=1L),
                             tf$concat(list(tf$matrix_transpose(K_star_13), tf$matrix_transpose(K_star_23), K_star_33), axis=1L)
                             ), axis=0L)
