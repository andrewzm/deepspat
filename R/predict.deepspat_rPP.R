#' @title Predict fitted dependence from an r-Pareto process model
#' @description Computes warped coordinates and fitted CEP dependence maps from a fitted \code{deepspat_rPP} object.
#' @param object a fitted \code{deepspat_rPP} object
...
#' @param uprime threshold used by the WLS CEP uncertainty assessment
...
predict.deepspat_rPP <- function(object, newdata,
  ...
  if (se) {
    ...
    if (d$method == "GSM") {
      ...
      if (is.null(uprime)) {
        stop("`uprime` must be provided when `se = TRUE` for WLS rPP fits.",
             call. = FALSE)
      }
    ...
    if (d$method == "GSM") {
      ...
      K <- as.matrix(K_tf)
      Jinv <- solve(J)
      Sigma.psi <- Jinv %*% K %*% t(Jinv) / dim(d$z_tf)[2]
    } else if (d$method == "WLS") {
      ...
      cep.pairs_boot <- sapply(1:B, function(b) {
        samp_b <- sample(1:nrepli, nrepli, replace = TRUE)
