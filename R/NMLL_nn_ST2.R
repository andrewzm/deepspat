...
## using sparse tensor
logmarglik_nn_sepST_GP_reml_sparse2 <- function(s_tf, t_tf, X,
  ...
  t_in_extra <- tf$constant(seq(10,11,length.out=m), dtype = "float32", shape = c(m, 1L))
  t_in_new <- tf$concat(list(t_in, t_in_extra), axis=0L)
  ...
  t_in_new <- tf$gather(t_in_new, order_idx_new - 1L)
  ...
  t_tf <- tf$gather(t_in, 0:(n-1)) %>% tf$reshape(c(n, 1L, 1L))
  t_neighbor_tf <- tf$gather(t_in_new, idx - 1L) %>% tf$reshape(c(n, m, 1L))
  ...
  D <- (K3 - tf$matmul(tf$matmul(tf$linalg$matrix_transpose(K2), tf$matrix_inverse(K1)), K2)) %>% 
    tf$reshape(c(1L, n))
  #D0 <- tf$concat(list(D0, D), axis=1L)
