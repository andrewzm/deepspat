#' @title Predict fitted dependence from a max-stable process model
#' @description Computes warped coordinates and fitted extremal dependence maps from a fitted \code{deepspat_MSP} object.
#' @param object a fitted \code{deepspat_MSP} object
#' @param newdata data frame containing prediction locations
#' @param type optional prediction type: warped coordinates or fitted dependence map
#' @param reference reference-site index for \code{type = "dependence"}
#' @param se logical; if \code{TRUE}, return delta-method standard errors when available
#' @param edm_emp empirical extremal dependence estimates for WLS uncertainty assessment
#' @param ... currently unused
#' @return A list containing warped coordinates, fitted dependence parameters, and optionally a reference-site dependence map.
#' @export
predict.deepspat_MSP <- function(object, newdata,
  ...
  type <- if (is.null(type)) "warp" else
    match.arg(type, c("warp", "dependence"))
  d <- object
  dtype <- d$dtype
  family <- d$family

  mmat <- model.matrix(update(d$f, NULL ~ .), newdata)
  s_new_tf <- tf$constant(mmat, dtype = dtype, name = "s")
  s_new_in <- scale_0_5_tf(s_new_tf, d$scalings[[1]]$min,
                           d$scalings[[1]]$max, dtype)
  ...
  if (family == "power_stat") {
    ...
    if (d$nlayers > 1) for (i in 1:d$nlayers) {
      if (d$layers[[i]]$name == "LFT") {
        h_tf[[i + 1]] <- d$layers[[i]]$f(h_tf[[i]],
                                         d$layers[[i]]$trans(d$a_tf))
      } else {
  ...
  if (se) {
    ...
    if (d$method %in% c("MPL", "MRPL")) {
      ...
      Jprime <- as.matrix(J_tf)
      J <- Jprime / p1
      Iprime <- as.matrix(I_tf)
      I <- Iprime / (p1^2)
      Jinv <- solve(J)
      ...
      uklall_tf <- 1 / tf$maximum(zk_tf, zl_tf)
      uklall_mean <- tf$reduce_mean(uklall_tf, axis = 1L, keepdims = TRUE)
      ukl_centered <- uklall_tf - uklall_mean
      theta_all <- nrepli / tf$reduce_sum(uklall_tf, axis = 1L)
      theta_all <- tf$clip_by_value(theta_all, clip_value_min = 1, clip_value_max = 2)
      thetaSq_all <- tf$expand_dims(theta_all^2, axis = 1L)
      ...
      body <- function(i_idx, G2) {
        ...
        end_id <- min(batch_size * as.integer(i_idx) + batch_size,
                      npairs * (npairs - 1) / 2)
        ...
        cov_theta <- tf$reduce_sum(thetaSq_uc_1 * thetaSq_uc_2,
  ...
  warp_out <- list(original = as.matrix(mmat),
                   srescaled = as.matrix(s_new_in),
  ...
  dep_df <- deepspat_extreme_dependence_map(swarped = as.matrix(s_new_out),
                                             reference = reference,
                                             phi = fitted.phi,
                                             kappa = fitted.kappa,
                                             model = "MSP",
  ...
  out <- c(warp_out, list(df_dependence = dep_df))
  out
}
